...
import (
	...
	"github.com/setherplatform/sether-node/ethapi"
	...
	"github.com/setherplatform/sether-node/gossip/blockproc/verwatcher"
	...
	"github.com/setherplatform/sether-node/gossip/proclogger"
	snapsync "github.com/setherplatform/sether-node/gossip/protocols/snap"
	"github.com/setherplatform/sether-node/inter"
	"github.com/setherplatform/sether-node/logger"
	"github.com/setherplatform/sether-node/utils/signers/gsignercache"
	"github.com/setherplatform/sether-node/utils/wgmutex"
	"github.com/setherplatform/sether-node/valkeystore"
...
func newService(config Config, store *Store, blockProc BlockProc, engine lachesis.Consensus, dagIndexer *vecmt.Index, newTxPool func(evmcore.StateReader) TxPool) (*Service, error) {
	...
	var err error
	svc.operaDialCandidates, err = dnsclient.NewIterator(config.SetherDiscoveryURLs...)
	if err != nil {
		return nil, err
	}
...
// MakeProtocols constructs the P2P protocol definitions for `sether`.
...
func (s *Service) Stop() error {
	defer log.Info("Sether node service stopped")
	s.verWatcher.Stop()
	for _, em := range s.emitters {
		em.Stop()
	}
