...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.Store, error) {
	...
	dm.diskSizeGauge = metrics.GetOrRegisterGauge("sether/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/size", nil)
	if strings.HasPrefix(name, "gossip-") || strings.HasPrefix(name, "lachesis-") {
		name = "epochs"
	}
	logger := log.New("database", name)
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("sether/chaindata/"+name+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("sether/chaindata/"+name+"/disk/write", nil)
