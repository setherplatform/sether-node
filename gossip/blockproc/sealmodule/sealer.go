...
type SetherEpochsSealerModule struct{}

func New() *SetherEpochsSealerModule {
	return &SetherEpochsSealerModule{}
}

func (m *SetherEpochsSealerModule) Start(block iblockproc.BlockCtx, bs iblockproc.BlockState, es iblockproc.EpochState) blockproc.SealerProcessor {
	return &SetherEpochsSealer{
		block: block,
		es:    es,
		bs:    bs,
	}
...
type SetherEpochsSealer struct {
	block iblockproc.BlockCtx
	es    iblockproc.EpochState
	bs    iblockproc.BlockState
}

func (s *SetherEpochsSealer) EpochSealing() bool {
...
func (p *SetherEpochsSealer) Update(bs iblockproc.BlockState, es iblockproc.EpochState) {
	p.bs, p.es = bs, es
}

// SealEpoch is called after pre-internal transactions are executed
func (s *SetherEpochsSealer) SealEpoch() (iblockproc.BlockState, iblockproc.EpochState) {
